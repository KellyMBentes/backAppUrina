...
class AccountTests(APITestCase):
    def test_score(self):
        ...
        data = {'email': 'score@gmail.com', 'password': '123', 'confirm_password': '123'}
        response = self.client.post(url_register, data, format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(CustomUser.objects.get().email, 'score@gmail.com')

        # Login

        user = CustomUser.objects.get(email='score@gmail.com')
        user.is_active = True
        user.save()
        url_login = "http://127.0.0.1:8000/api/user/login/"
        data = {'email': 'score@gmail.com', 'password': '123'}
        ...
        url = "http://127.0.0.1:8000/api/score/1"
